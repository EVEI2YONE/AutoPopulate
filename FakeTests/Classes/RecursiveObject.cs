namespace FakeTests.Classes
{
    public class RecursiveObject : ITestableObject
    {
        ...
        public bool ItemsSuccessfullyPopulated(int? depth = 1)
        {
            ...
            if (depth <= 1 && (!recursiveObject_A?.ItemsSuccessfullyPopulated(++depth) ?? false)) return false;
            if (depth <= 1 && (!recursiveObject_B?.ItemsSuccessfullyPopulated(++depth) ?? false)) return false;
            return true;
        }
    }
}
